...
{   // 计算进度
    if (collection.subject_type == 2) {
        ...
        const QJsonArray episodes = airdatesJson.value(QString::number(collection.subject_id)).toArray();
    ...
    if (collection.subject_type == 7 || collection.subject_type == 8 || collection.subject_type == 1) return QString("全 %1 卷 · 全 %2 话").arg(collection.subject_volumes > 0 ? QString::number(collection.subject_volumes) : "--", collection.subject_eps > 0 ? QString::number(collection.subject_eps) : "--");
    if (collection.subject_type == 4) return QString("%1 发售").arg(collection.subject_date.isNull() ? "--" : collection.subject_date);
    return {};
}